...
const Manager = () => {
    ...
    const [setupForm, setSetupForm] = useState({ masterPassword: "", confirmMasterPassword: "" });
    ...
    const strength = useMemo(() => estimateStrength(form.password), [form.password]);

    const categoryOptions = useMemo(() => {
        const categories = new Set(credentials.map((item) => item.category || "General"));
    ...
    const stats = useMemo(() => {
        ...
        const categories = new Set(credentials.map((item) => item.category || "General")).size;
    ...
    const handleSetupSubmit = async (event) => {
        ...
        try {
            ...
            const unlockResponse = await api.auth.unlock({ masterPassword: setupForm.masterPassword });
    ...
    const handleUnlockSubmit = async (event) => {
        ...
        try {
            const response = await api.auth.unlock({ masterPassword: unlockPassword });
        ...
        return (
            <section className="vault-panel">
                ...
                <p className="vault-subtitle">
                    Set one strong master password. It unlocks your encrypted credential store.
                </p>
                <form onSubmit={handleSetupSubmit} className="vault-form">
                    <label>
                        ...
                        <input
                            ...
                            onChange={(event) =>
                                setSetupForm((current) => ({ ...current, masterPassword: event.target.value }))
                            }
                            required
                            minLength={8}
                    ...
                    <label>
                        ...
                        <input
                            ...
                            onChange={(event) =>
                                setSetupForm((current) => ({ ...current, confirmMasterPassword: event.target.value }))
                            }
                            required
                            minLength={8}
    ...
    const renderVaultUnlock = () => {
        return (
            <section className="vault-panel">
                ...
                <p className="vault-subtitle">Enter your master password to access credentials.</p>
            ...
            <section className="manager-shell">
                <header className="manager-header">
                    <div>
                        ...
                        <p>Manage, search, and rotate credentials with encrypted storage.</p>
                    </div>
                ...
                {legacyImportNeeded && (
                    <div className="legacy-callout">
                        <div>
                            ...
                            <p>Import passwords from your previous localStorage-based manager.</p>
                        </div>
                        <button type="button" className="btn-primary" onClick={importLegacy}>
                            Import legacy data
                        </button>
                    </div>
                )}
                ...
                <div className="workspace-grid">
                            ...
                            <label>
                                ...
                                <div className="password-row">
                                    <input
                                        ...
                                        placeholder={editingId ? "Set a new password only if needed" : "Enter strong password"}
                                        required={!editingId}
                                    />
                                    <button
                                        type="button"
                    ...
                    <section className="list-card">
                        ...
                        {isLoadingCredentials ? (
                            ...
                            <p className="empty-hint">No credentials found for current filters.</p>
                        ) : (
                            <>
                                                    ...
                                                    <td>
                                                        ...
                                                        {revealedSecrets[item.id] && (
                                                            <button
                                                                type="button"
                                                                className="btn-link"
                                                                onClick={() => copyText(revealedSecrets[item.id], "Password")}
                                                            >
                                                                Copy
                                                            </button>
                                                        )}
                                ...
                                <div className="mobile-cards">
                                    {credentials.map((item) => (
                                        <article key={item.id}>
                                            ...
                                            <div className="action-row">
                                                <button type="button" className="btn-link" onClick={() => toggleSecret(item.id)}>
                                                    {revealedSecrets[item.id] ? "Hide" : "Reveal"}
                                                </button>
                                                {revealedSecrets[item.id] && (
                                                    <button
